...
void attitude_thread(void *p1, void *p2, void *p3) {
    ...
    while (1) {
        ...
        if (prev_time_ticks == 0) {
            delta = 0;
            prev_time_ticks = k_ticks_to_us_floor64(imu_message.timestamp);
        }
        else {
        ...
        am.roll = atan2f(2.0f*((quants.q1 * quants.q2) + (quants.q3*quants.q4)), 1.0f - 2.0f*(quants.q1 * quants.q1 + quants.q3 * quants.q3)) * RAD_TO_DEG;
        am.pitch = asinf(2.0f*(quants.q1*quants.q3 - quants.q4*quants.q2)) * RAD_TO_DEG;
        am.yaw = atan2f(2.0f * (quants.q1 * quants.q4 + quants.q2 * quants.q3), 
                1.0f - 2.0f * (quants.q3 * quants.q3 + quants.q4 * quants.q4)) * RAD_TO_DEG;
